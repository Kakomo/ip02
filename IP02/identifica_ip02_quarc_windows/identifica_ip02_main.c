...
#include "ext_work.h"
#include "quanser_reconfiguration.h"
...
typedef identifica_ip02_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * identifica_ip02(void);
...
static const t_uint invalid_reconfiguration_group = ~0U;
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(identifica_ip02_M, GBLbuf.message);
}
...
static void
  cleanup_reconfiguration(void * argument)
{
  t_reconfiguration reconfiguration = (t_reconfiguration) argument;
  reconfiguration_close(reconfiguration);
}
  ...
  main(int argc, char * argv[])
{
  ...
  qsigaction_t action;
  t_uint reconfiguration_group = invalid_reconfiguration_group;
  t_uint switch_out_group = 0;
  t_boolean is_switching = false;
  ...
  for (count = 1; count < argc; ) {
    ...
    if ((strcmp(option, "-tf") == 0) && (count != argc)) {/* final time */
      ...
      argv[count-2] = NULL;
      argv[count-1] = NULL;
    } else if ((strcmp(option, "-reconfig") == 0) && (count != argc)) {/* reconfiguration group */
      const char * reconfig_argument = argv[count++];
      int group_id;                    /* use an int for the sscanf since int_T may be the wrong size depending on the platform */
      int items = sscanf(reconfig_argument, "%d%1s", &group_id,
                         extraneous_characters);
      if (items != 1 || group_id < 0) {
        (void) fprintf(stderr,
                       "reconfiguration group must be a positive, integral value.\n");
        parseError = true;
        break;
      }

      reconfiguration_group = (t_uint)group_id;
      argv[count-2] = NULL;
      argv[count-1] = NULL;
    } else if ((strcmp(option, "-switch") == 0) && (count != argc)) {/* switch model in */
      const char * reconfig_argument = argv[count++];
      int group_id;                    /* use an int for the sscanf since int_T may be the wrong size depending on the platform */
      int items = sscanf(reconfig_argument, "%d%1s", &group_id,
                         extraneous_characters);
      if (items != 1 || group_id < 0) {
        (void) fprintf(stderr,
                       "reconfiguration group for switching must be a positive, integral value.\n");
        parseError = true;
        break;
      }

      switch_out_group = (t_uint)group_id;
      is_switching = true;
      argv[count-2] = NULL;
      argv[count-1] = NULL;
    }
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: identifica_ip02 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    (void) fprintf(stderr,
                   "\t-reconfig 1           - set reconfiguration group to 1\n");
    (void) fprintf(stderr,
                   "\t-switch 0             - switch out model in group 0\n");
    (void) fprintf(stderr, "\n");
    return (exit_failure);
  }
  ...
  result = reconfiguration_open(&_reconfiguration);
  if (result < 0) {
    msg_get_error_messageA(NULL, result, GBLbuf.message, sizeof(GBLbuf.message));
    (void) fprintf(stderr, "Unable to open a reconfiguration instance. %s\n",
                   GBLbuf.message);
    return (exit_failure);
  }

  qthread_cleanup_push(cleanup_reconfiguration, _reconfiguration);
  reconfiguration_set_switching_in(_reconfiguration, switch_out_group,
    is_switching);
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      if (reconfiguration_group != invalid_reconfiguration_group)
        reconfiguration_set_group(_reconfiguration, reconfiguration_group);
      result = reconfiguration_begin_control(_reconfiguration);
      if (result == 0) {
        result = hil_task_start
          (identifica_ip02_DWork.EncoderInput0CartPositionEnco_m,
           identifica_ip02_P.EncoderInput0CartPositionEncode, 1000.0, -1)
          ;
        if (result == 0) {
          ...
          while (true) {
            ...
            if (reconfiguration_is_switching_out(_reconfiguration)) {
              rtmSetStopRequested(S, 1);
            }
          ...
          /* disarm the timebase */
          hil_task_stop(identifica_ip02_DWork.EncoderInput0CartPositionEnco_m)
            ;
        ...
        reconfiguration_end_control(_reconfiguration);
      } else {
        msg_get_error_messageA(NULL, result, GBLbuf.submessage, sizeof
          (GBLbuf.submessage));
        string_format(GBLbuf.message, sizeof(GBLbuf.message),
                      "Unable to begin dynamic reconfiguration. %s",
                      GBLbuf.submessage);
        rtmSetErrorStatus(S, GBLbuf.message);
      }
