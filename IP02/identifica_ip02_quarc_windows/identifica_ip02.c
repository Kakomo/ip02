...
void identifica_ip02_output(int_T tid)
{
...

  {
    ...
    if (rtmIsMajorTimeStep(identifica_ip02_M) &&
      ...
      if (rtmIsMajorTimeStep(identifica_ip02_M) &&
        ...
        if (identifica_ip02_DWork.CartPositionWatchdog_MODE == SUBSYS_ENABLED) {
          /* RelationalOperator: '<S4>/Check Maximum Allowed Position' incorporates:
           *  Constant: '<S4>/X MAX'
           */
          ...
          /* Stop Block: '<S6>/Stop Simulation'
           */
          if (identifica_ip02_B.Compare) {
            rtmSetStopRequested(identifica_ip02_M, 1);
          }

          /* RelationalOperator: '<S4>/Check Minimum Allowed Position' incorporates:
           *  Constant: '<S4>/X IN'
           */
    ...
    identifica_ip02_B.IP02ModeloExperimental =
      ...
      identifica_ip02_X.IP02ModeloExperimental_CSTATE;
    if (rtmIsMajorTimeStep(identifica_ip02_M) &&
        identifica_ip02_M->Timing.TaskCounters.TID[1] == 0) {
    }
    ...
    if (identifica_ip02_P.Constant_Value_p >
      ...
      identifica_ip02_B.SwitchControl = identifica_ip02_B.MultiportSwitch;
    } else {
      identifica_ip02_B.SwitchControl = 0.0;
    }

    /* Sum: '<Root>/Sum1' */
    identifica_ip02_B.Sum1 = identifica_ip02_B.SwitchControl -
      identifica_ip02_B.IP02ModeloExperimental;

    /* Gain: '<Root>/Kp1' */
    identifica_ip02_B.Kp1 = identifica_ip02_P.Kp1_Gain * identifica_ip02_B.Sum1;
...
void identifica_ip02_initialize(boolean_T firstTime)
{
  ...
  rtmSetTFinal(identifica_ip02_M, 1.0);
  identifica_ip02_M->Timing.stepSize0 = 0.001;
  identifica_ip02_M->Timing.stepSize1 = 0.001;

  /* external mode info */
  identifica_ip02_M->Sizes.checksums[0] = (3669811370U);
  identifica_ip02_M->Sizes.checksums[1] = (1029227055U);
  identifica_ip02_M->Sizes.checksums[2] = (1059536654U);
  identifica_ip02_M->Sizes.checksums[3] = (78256845U);
...
{
  identifica_ip02_M->Sizes.numContStates = (1);/* Number of continuous states */
  ...
  identifica_ip02_M->Sizes.numBlocks = (54);/* Number of blocks */
  identifica_ip02_M->Sizes.numBlockIO = (34);/* Number of block outputs */
  identifica_ip02_M->Sizes.numBlockPrms = (156);/* Sum of parameter "widths" */
}
...
void MdlInitialize(void)
{
  ...
  /* TransferFcn Block: '<Root>/IP02 Modelo Experimental' */
  identifica_ip02_X.IP02ModeloExperimental_CSTATE = 0.0;
}
